...
def output_type_string(type_string: str) -> str:
    return type_string.replace('int', 'int64').replace('float', 'float64')
...
class EvalVisitor(GuaVisitor):
    ...
    def visitLabelStatementDefinition(self, ctx: GuaParser.LabelStatementDefinitionContext):
        ...
        if ctx.VALUETYPE() is not None:
            ...
            _type = output_type_string(ctx.LISTTYPE().getText())
            self.cur_block()[k] = container_value_type_string(_type)
            return string_with_space(self.num, f'var {k} {_type} = {_type}{v}\n')
            ...
            return string_with_space(self.num, f'var {k} {type_string} = {v}\n')
    ...
    def visitLabelStatementWhile(self, ctx: GuaParser.LabelStatementWhileContext):
        ...
        result = string_with_space(self.num, f'for {check} {{\n')
        result += block
    ...
    def visitLabelStatementFunction(self, ctx: GuaParser.LabelStatementFunctionContext):
        ...
        if self.current_class != '':
            ...
            if return_parameter is None:
                result = string_with_space(self.num, f'func {name}({parameter}) {{\n')
            else:
                return_parameter = self.visit(return_parameter)
                result = string_with_space(self.num, f'func {name}({parameter}) ({return_parameter}) {{\n')
    ...
    def visitLabelStatementClass(self, ctx: GuaParser.LabelStatementClassContext):
        ...
        result = string_with_space(self.num, f'type {name} struct {{\n')
        self.num += 1
        for k, v in field_map.items():
            result += string_with_space(self.num, f'{k} {v}\n')
        self.num -= 1
        result += string_with_space(self.num, f'}}\n')
        result += block
    ...
    def visitFormalParameters(self, ctx: GuaParser.FormalParametersContext):
        ...
        for i in range(length):
            ...
            v = output_type_string(ctx.VALUETYPE(i).getText())
            self.temp_block[k] = v
            es.append(k + ' ' + v)
        return ', '.join(es)
    ...
    def visitReturnParameters(self, ctx: GuaParser.ReturnParametersContext):
        es = []
        for e in ctx.VALUETYPE():
            r = output_type_string(e.getText())
            es.append(r)
        return ', '.join(es)
...
def main():
    ...
    with open("../test-go/create.go", "w") as f:
